...
void *process_read_write(long tid, long obno,  int count, char mode){
  ...
  // get the transaction id first before acquiring the lock
  zgt_tx *tx = get_tx(tid);  
  tx->set_lock(tid, 1, obno, count, mode);
...
int zgt_tx::end_tx()  
{
  ...
  //Initialize the linktx and prevp pointers to the last node in the linked list
  linktx = prevp = ZGT_Sh->lastr;
  
  //Traverse the linked list using a while loop, looking for the first node with a tid value equal to this->tid. 
  ...
  //If linktx is NULL, it means the node with the desired tid was not found in the list. 
  //In this case, print an error message and return 1 to indicate failure.
  if (linktx == NULL) {
    printf("\ncannot remove a Tx node; error\n");
    fflush(stdout);
    return (1);
  }
  //Check if linktx is the last node in the list (ZGT_Sh->lastr). If so, 
  //update the last node pointer to the next node in the list.
  if (linktx == ZGT_Sh->lastr) ZGT_Sh->lastr = linktx->nextr;
  //If linktx is not the last node, find the previous node (prevp) in the list and 
  //update its nextr pointer to skip linktx, effectively removing linktx from the list
  else {
    prevp = ZGT_Sh->lastr;
