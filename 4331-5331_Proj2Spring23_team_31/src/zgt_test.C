                      ...
                      const string& delimiters = " ")
{
    ...
    string::size_type pos     = str.find_first_of(delimiters, lastPos);
        int i =0;
    //reading till last and adding tokenlist[]
...
int main(int argn, char **argv){
  ...
  // from terminal, geting argument.
  infilename = argv[1];
  ifstream inFile(infilename,ios::in);

/// check if the file is read by any other process
if (! inFile.is_open())
...
//also the hash table used as lock table
 ...
 ZGT_Ht = new zgt_ht(ZGT_DEFAULT_HASH_TABLE_SIZE);
    ...
    {   
	 ...
	 printf("BeginTx : %d\n\n", tid);
	 printf("TxType : %c\n\n", Txtype);
   // checking transaction with that id is still exit
